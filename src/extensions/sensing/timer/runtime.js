...
export function installTimer(core) {
    ...
    const initPrec = core.settings?.get?.('timer.precision') ?? 2;
    el.textContent = `00.${'0'.repeat(initPrec)}`;
    ...
    core.app.ticker.add(() => {
        ...
        const fraction = Math.floor((val - sec) * Math.pow(10, prec));
        el.textContent = `${String(sec).padStart(2, "0")}.${String(fraction).padStart(prec, "0")}`;
    });
}
